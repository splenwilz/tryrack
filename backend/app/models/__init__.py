...
class VirtualTryOnResult(Base):
    ...
    # Item information (legacy single-item fields, kept for backward compatibility)
    ...
    # Multi-item support: JSON array of items being tried on
    # Format: [{"item_id": "123", "item_type": "wardrobe", "category": "top", "colors": ["blue"]}, ...]
    # Nullable to support existing records; new try-ons should populate this field
    items = Column(JSON, nullable=True)
    
    # Image URLs
    user_image_url = Column(String(500), nullable=False)  # User's photo
    item_image_url = Column(String(500), nullable=False)  # Item image (legacy, for single-item compatibility)
...
class RefreshToken(Base):
    """
    Refresh token model for OAuth2-style token refresh flow.
    
    Industry standard implementation:
    - Long-lived refresh tokens (7-30 days) used to obtain new short-lived access tokens
    - Token rotation: new refresh token issued on each refresh, old one revoked
    - Secure: stored in database, can be revoked independently of access tokens
    """
    ...
    revoked = Column(Boolean, default=False, nullable=False, index=True)  # Token revocation flag
    created_at = Column(DateTime(timezone=True), server_default=func.now(), nullable=False)
    ...
    __table_args__ = (
        {'comment': 'Refresh tokens for secure token refresh flow'}
    )